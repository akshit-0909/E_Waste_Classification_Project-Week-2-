...
#Adjust layout and display all images at once
plt.tight_layout()
plt.savefig("Outputs/random_batch.png")
plt.show(block=False)
plt.pause(5)
plt.close('all') # Ensures the window doesn't block further code Execution

def plot_class_distribution(dataset, title="Class Distribution", save_dir="Outputs"):
...
# Data Preprocessing/prepration (To improve generalization)
...
#Model Selection (Custom CNN or Transfer Learning eg :- EfficientNet2B0)
...
# Model Training, Model tuning & Optimization
...
# (Callbacks) Define an EarlyStopping callback to stop training when validation loss stops improving
...
print("Printing the architecture summary of the  model :-")
model.summary() # Print the architecture summary of the  model
print("Print the architecture summary of the base model :-")
base_model.summary() # Print the architecture summary of the base model

### Plotting Training and Validation Accuracy and Loss Over Epochs
print("Plotting Training and Validation Accuracy and Loss Over Epochs :-")
...
# Model Evaluation (Evaluate model performance on validation or test set.)
...
plt.savefig("Outputs/Cofusion_matrix.png")
plt.show(block=False)
plt.pause(5)
plt.close('all') # Ensures the window doesn't block further code Execution                                          
...
import os
save_dir = "Outputs/SamplePredictions"
os.makedirs(save_dir, exist_ok=True)
...
for images, labels in datatest.take(1):                                                   # Take one batch from test data
    ...
    for i in range(8):                                                                    # Display first 8 images from batch
        ...
        filename = os.path.join(save_dir, f"sample_prediction_{i+1}.png")
